...
function ParticipantList({ onMinimizedChange }: ParticipantListProps) {
  ...
  const handleDelete = async (luckyNumber: string) => {
    ...
    if (window.confirm('Bạn có chắc chắn muốn xóa số đăng ký này?')) {
      try {
        const participantRef = ref(db, `registration/${luckyNumber}`)
        await remove(participantRef)
      } catch (error) {
        console.error('Error deleting participant:', error)
        alert('Có lỗi xảy ra khi xóa số đăng ký')
      }
    }
  }
          ...
          {isLoading ? (
            ...
            <div className="participant-list-content">
              {sortedParticipants.map((participant) => {
                const isRegistered = participant.luckyNumber !== 'not registered yet'
                return (
